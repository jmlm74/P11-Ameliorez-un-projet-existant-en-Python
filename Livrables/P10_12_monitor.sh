...
# https://www.cyberciti.biz/tips/processing-the-delimited-files-using-cut-and-awk.html
# ---------------------------------------------------
# Last updated: Jun - 15 - 2009.
ports="4321 80 443 5432"
 
# service names as per above ports
service="SSH HTTP HPTTPS POSTGRES"
 
#Email id to send alert
ADMINEMAIL="jmlm74@gmail.com"
...
chk_services(){
	log "subject: Attention : service down @ $(hostname)"
	log "-------------------------------------------------------------"
	log "Running services status @ $(hostname) [ $(date) ]"
	log "-------------------------------------------------------------"
 
	# get open ports 
#	RPORTS=$($NETSTAT -tulpn -A inet,inet6 | grep -vE '^Active|Proto' | grep 'LISTEN' | awk '{ print $4}' | cut -d: -f2 | sed '/^$/d' | sort  -u)
	RPORTS=$($NETSTAT -tulpn -A inet6 | grep -vE '^Active|Proto' | grep 'LISTEN' | awk '{ print $4}' | cut -c4- | sed '/^$/d' | sort  -u)
